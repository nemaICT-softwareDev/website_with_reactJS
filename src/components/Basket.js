...
import { addToCart, handleSingleRemove, handleTotalRemove, sortSelectedProducts} from "../actions/cartActions";
                ...
                {!cartItems.length ?
                    ...
                    <div className="infoBasket">
                        <p className={"userBasketInfo"}>Lucky bird! you have&nbsp;
                            ...
                            <span className="glyphicon glyphicon-shopping-cart basket"/></p>
                        <hr/>
                    </div>}
                {cartItems.length > 0 &&
                <div>
                        ...
                        {cartItems.map((item) => <li key={item.id} className="list-unstyled">
                            <div className="form-check">
                                <button type={"button"} className={"btn btn-outline-warning"} value={"Delete"}
                                        onClick={() => {
                                            this.props.handleSelectedRemove(this.props.cartItems, item)
                                        }}>
                                    Delete
                                </button>
                                &nbsp;&nbsp;&nbsp;
...
export default connect(mapStateToProps, { addToCart , removeFromCart: handleSingleRemove, handleSelectedRemove: handleTotalRemove, sortSelectedProducts})(Basket);