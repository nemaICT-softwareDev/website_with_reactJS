        ...
        const productItems = this.props.products.map( (product) => (
                <div className="col-md-4" key={product.id}>
                     <div className="thumbnail">
                               <a>
                                 <div className={"prodDescription"}>{product.title}</div>
                                   <img className={"image"} src={`products/${product.sku}.jpg`} alt={product.title} title={product.description}/>
                                 <div className={"pPrice"}>{util.formatCurrency(product.price)}</div>
                               </a><hr/>
                        <div className="btn-group btn-group-md btn-group-justified">
                            <button type="button" className="btn btn-outline-warning glyphicon glyphicon-shopping-cart pButtons"
                                    onClick={() => {this.props.addToCart(this.props.cartItems, product)}}/>
                            <button type="button" className="btn btn-outline-warning glyphicon glyphicon-minus"
                                    onClick={() =>{this.props.handleSingleRemove(this.props.cartItems, product)}}/>
                            <button type="button" className="btn btn-outline-warning glyphicon glyphicon-plus pButtons"
                                    onClick={() => {this.props.addToCart(this.props.cartItems, product)}}/>
                        </div>
                     </div>
                </div>
